...
if ($env:SEARCH_CONFIG_FILE) {
  $SearchConfigFile = $env:SEARCH_CONFIG_FILE
} elseif (Test-Path (Join-Path $PSScriptRoot 'config/search-profiles.conf')) {
  $SearchConfigFile = Join-Path $PSScriptRoot 'config/search-profiles.conf'
} else {
  $SearchConfigFile = Join-Path $PSScriptRoot '../../config/search-profiles.conf'
}
